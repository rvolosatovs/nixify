...
let
  ...
  f =
    ...
    let
      ...
      includeCrate =
        if build'.workspace || !isPackage then
          true
        else if packagesSelected then
          any (p: p == cargoToml.package.name) build'.packages
        else
          true;
      ...
      normalise' =
        p:
        if hasPrefix "/" p then
          throw "absolute paths not supported"
        else
          let
      ...
      collectPathDeps =
        attrs:
        let
          depAttrs = filterAttrs (
            k: _: k == "build-dependencies" || k == "dependencies" || k == "dev-dependencies"
          ) attrs;
      ...
      pathDeps =
        ...
        ++ optionals (cargoToml ? target) (
          flatten (attrValues (mapAttrs (_: v: collectPathDeps v) cargoToml.target))
        )
    ...
    trace' "crateBins"
      ...
      (
        ...
        attrValues (
          ...
          // listToAttrs (
            optionals (autobins && pathExists "${src}/src/bin") (
              map (name: nameValuePair "src/bin/${name}" (removeSuffix ".rs" name)) (
                attrNames (
                  filterAttrs (name: type: type == "regular" && hasSuffix ".rs" name || type == "directory") (
                    readDir "${src}/src/bin"
                  )
                )
              )
          ...
          // listToAttrs (
            map (
              ...
              else if autobins && pathExists "${src}/src/main.rs" && name == cargoToml.package.name then
                nameValuePair "src/main.rs" name
              else if autobins && pathExists "${src}/src/bin/${name}.rs" then
                nameValuePair "src/bin/${name}.rs" name
              else if autobins && pathExists "${src}/src/bin/${name}/main.rs" then
                nameValuePair "src/bin/${name}/main.rs" name
              else
                throw "failed to determine `${name}` binary path, please file a bug report and explicitly set `path` in `Cargo.toml` to temporarily work around this issue"
            ) bin
          )
        )
      )
...
in
if build ? bins && length build.bins > 0 then build.bins else unique (f src)
