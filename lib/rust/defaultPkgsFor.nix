...
in
if hostPlatform.config == target then
  pkgs
else if
...
then
  pkgs
else if hostPlatform.isAarch64 && hostPlatform.isLinux && target == aarch64-unknown-linux-musl then
  if hostPlatform.isMusl then pkgs else pkgs.pkgsCross.aarch64-multiplatform-musl
else if
...
then
  pkgs
else if hostPlatform.isx86_64 && hostPlatform.isLinux && target == x86_64-unknown-linux-musl then
  if hostPlatform.isMusl then pkgs else pkgs.pkgsCross.musl64
else if hostPlatform.isx86_64 && hostPlatform.isWindows && target == x86_64-pc-windows-gnu then
  pkgs
else if target == aarch64-apple-darwin then
  pkgs.pkgsCross.aarch64-darwin
else if target == aarch64-apple-ios then
  pkgs.pkgsCross.iphone64
...
else if target == arm-unknown-linux-gnueabihf then
  pkgs.pkgsCross.raspberryPi
else if target == arm-unknown-linux-musleabihf then
  pkgs.pkgsCross.muslpi
else if target == armv7s-apple-ios then
  ...
  pkgs.pkgsCross.mips64-linux-gnuabi64
...
else if target == s390x-unknown-linux-musl then
  pkgs.pkgsCross.s390x
else if target == x86_64-apple-darwin then
  pkgs.pkgsCross.x86_64-darwin
else if target == x86_64-apple-ios then
  pkgs.pkgsCross.iphone64-simulator
...
else if target == wasm32-unknown-unknown || target == wasm32-wasip1 || target == wasm32-wasip2 then
  pkgs.pkgsCross.wasi32
else
  import (if pkgs.stdenv.buildPlatform.isDarwin then nixpkgs-darwin else nixpkgs-nixos) {
    crossSystem.config =
      if target == riscv64gc-unknown-linux-musl then "riscv64-unknown-linux-musl" else target;
